...
\]
This is implemented in circuit form as:
\[
D = H^{\otimes n}(2|0\rangle\langle 0| - I)H^{\otimes n}
\]
using Hadamard gates, Pauli-X gates, and a multi-controlled-Z gate. Together, $U_f$ and $D$ form one Grover iteration $G = D \cdot U_f$.
...
\textbf{CPU backend.} \texttt{AerSimulator(method=`statevector')} with OpenMP multithreading enabled. Memory was tracked via \texttt{psutil} RSS.

\textbf{GPU backend.} The GPU simulator uses the statevector method with CUDA acceleration, via \texttt{qiskit-aer-gpu} v0.15.1. Correct GPU dispatch was confirmed by inspecting the result metadata. Local GPU: NVIDIA RTX 4060 Laptop (8\,GB VRAM). HPC GPU: NVIDIA Tesla V100 (32\,GB HBM2).
...
\begin{table}[htbp]
\centering
\caption{Measured vs. Theoretical Memory Consumption and Execution Time on CPU}
...
The iteration sweep at $n=5$ ($N=32$, $\theta \approx 10.24^\circ$) confirmed the sinusoidal $P(k)$ formula:

\begin{table}[htbp]
\centering
\caption{Success Probability vs. Iteration Count ($n=5$)}
...
The optimal iteration count $k_{\text{opt}} = \lfloor\frac{\pi}{4}\sqrt{32}\rfloor = 4$ yielded 99.95\% success probability, matching theory to within 0.03\%. The overshoot at $k=8$ is dramatic: success probability collapses to 1.8\%, corresponding to the state having rotated past $|x^*\rangle$ by nearly $90^\circ$. Recovery at $k=12$ and partial decay at $k=15$ follow the sinusoidal prediction closely, validating both the formula and the geometric picture.
...
\begin{table}[htbp]
\centering
\caption{Oracle Query Complexity: Classical vs. Grover Search}
...
\begin{table}[htbp]
\centering
\caption{Transpiled Circuit Depth and Gate Counts}
...
\begin{table}[htbp]
\centering
\caption{Execution Time Comparison: Local CPU vs. Local GPU}
\label{tab:local_gpu_cpu}
\begin{tabular}{rrrl}
\toprule
...
For $n \leq 13$, the GPU's initialization and PCIe transfer overhead (approximately 1--2\,s fixed cost) outweighs the benefit of parallel gate application. Beyond $n=14$, the statevector size exceeds 256\,MB and the GPU's $9\times$ memory bandwidth advantage over DDR4 becomes decisive. Local GPU VRAM was exhausted at $n=24$ ($8\,\text{GB} \approx 2^{29}$ bytes for the statevector alone).
...
\begin{table}[htbp]
\centering
\caption{HPC CPU Simulation Scalability (AION Cluster)}
\label{tab:hpc_cpu}
\begin{tabular}{rrl}
\toprule
$n$ & Time (s) & Notes \\
...
\begin{table}[htbp]
\centering
\caption{HPC GPU Simulation Performance and Speedup (Tesla V100)}
\label{tab:hpc_gpu}
\begin{tabular}{rrrl}
\toprule
...
\begin{table}[htbp]
\centering
\caption{Statevector Memory Scaling Requirements}
\label{tab:memory_wall}
\begin{tabular}{rrl}
\toprule
$n$ & Amplitudes & Memory \\
\midrule
...
\bibitem{Haener2017} T. H\"{a}ner and D. S. Steiger. 0.5 petabyte simulation of a 45-qubit quantum circuit. \textit{Proceedings of SC17: International Conference for High Performance Computing}, 2017.
